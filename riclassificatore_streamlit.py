...
"""
================================================================================
RICLASSIFICATORE BILANCIO CEE - VERSIONE STREAMLIT PRONTA PER PRODUZIONE
...
class ParserDatiDinamico:
    ...
    def _estrai_conto_da_riga_excel(self, row) -> Optional[Dict]:
...
def mostra_tabella_bilancio(dati: Dict):
    try:
        ...
        st.error("Installare pandas per visualizzare la tabella.")
        return
    df_data = []
...
def main():
    ...
    if 'dati_bilancio' not in st.session_state:
        st.session_state['dati_bilancio'] = None
        ...
        if uploaded_file is not None:
            ...
            if st.button("🔄 Processa File", type="primary"):
                with st.spinner("Elaborazione in corso..."):
                    try:
                        mapping = st.session_state.get('custom_mapping')
                        parser = ParserDatiDinamico(mapping=mapping)
    ...
    with tab2:
        ...
        if st.session_state['dati_bilancio']:
            dati = st.session_state['dati_bilancio']
            col1, col2 = st.columns([3, 1])
            with col2:
                vista = st.selectbox(
                    "Tipo Vista",
                    ["Tabella Completa", "Riepilogo"]
                )
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: #666;'>
            <p>Riclassificatore Bilancio CEE - Powered by Streamlit</p>
